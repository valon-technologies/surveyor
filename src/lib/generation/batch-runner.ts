...
async function prepareEntityForRegeneration(
  ...
  batchRunId: string,
  milestone?: string,
): Promise<{ mappingsRetired: number; questionsDismissed: number; retiredMappingIds: string[] }> {
  ...
  let retiredMappingIds: string[] = [];
  if (fieldIds.length > 0) {
    // Find mapping IDs to retire (so we can rollback on generation failure)
    ...
    retiredMappingIds = toRetire.map(m => m.id);

    if (retiredMappingIds.length > 0) {
      await db
        .update(fieldMapping)
        .set({
          isLatest: false,
        ...
        .where(inArray(fieldMapping.id, retiredMappingIds));
      mappingsRetired = retiredMappingIds.length;
    }
  }
  ...
  try {
    ...
    for (const batch of orderedEntities) {
      ...
      try {
        ...
        retiredMappingIds = cleanup.retiredMappingIds;
        ...
        failedEntities++;
        console.error(`[batch] Entity "${batch.entityName}" failed:`, entityErr instanceof Error ? entityErr.message : entityErr);

        // Rollback: restore retired mappings so we don't lose data on failed generation
        if (retiredMappingIds && retiredMappingIds.length > 0) {
          try {
            await db.update(fieldMapping)
              .set({ isLatest: true, changeSummary: null, updatedAt: now() })
              .where(inArray(fieldMapping.id, retiredMappingIds));
            console.log(`[batch] Restored ${retiredMappingIds.length} retired mappings for "${batch.entityName}"`);
          } catch (restoreErr) {
            console.error(`[batch] CRITICAL: Failed to restore retired mappings for "${batch.entityName}":`, restoreErr);
          }
        }
      }
