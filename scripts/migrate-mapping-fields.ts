...
function run() {
  ...
  const closedCount = db.prepare(
    "UPDATE field_mapping SET status = 'accepted' WHERE status IN ('mapped', 'not_available', 'derived', 'default', 'system_generated', 'fully_closed')"
  ).run();
  console.log(`  mapped/not_available/derived/default/system_generated/fully_closed → accepted: ${closedCount.changes} rows`);

  const pendingCount = db.prepare(
    "UPDATE field_mapping SET status = 'unreviewed' WHERE status IN ('pending', 'requires_clarification', 'open_comment_sm', 'open_comment_vt')"
  ).run();
  console.log(`  pending/requires_clarification/open_comment_sm/open_comment_vt → unreviewed: ${pendingCount.changes} rows`);
