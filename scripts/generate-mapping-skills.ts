...
async function main() {
  ...
  const allContexts = await client`SELECT id, name FROM context` as CtxRow[];
  ...
  await client`DELETE FROM skill_context`;
  await client`DELETE FROM skill`;
...
6. Document any open questions for fields that can't be mapped`;
      ...
      await tx`
        ...
        VALUES (${crypto.randomUUID()}, ${skillId}, ${vdsCtxId}, 'primary', ${ctxOrder++},
      ...
      for (const table of pairing.sm_tables) {
        ...
        if (ctxId) {
          await tx`
            INSERT INTO skill_context (id, skill_id, context_id, role, sort_order, notes)
            VALUES (${crypto.randomUUID()}, ${skillId}, ${ctxId}, 'primary', ${ctxOrder++},
      ...
      for (const domain of pairing.sm_domains) {
        ...
        if (ctxId) {
          await tx`
            INSERT INTO skill_context (id, skill_id, context_id, role, sort_order, notes)
            VALUES (${crypto.randomUUID()}, ${skillId}, ${ctxId}, 'reference', ${ctxOrder++},
      ...
      if (categoryCtxId) {
        await tx`
          INSERT INTO skill_context (id, skill_id, context_id, role, sort_order, notes)
          VALUES (${crypto.randomUUID()}, ${skillId}, ${categoryCtxId}, 'reference', ${ctxOrder++},
      ...
      if (OVERVIEW_CTX.vds) {
        await tx`
          INSERT INTO skill_context (id, skill_id, context_id, role, sort_order, notes)
          VALUES (${crypto.randomUUID()}, ${skillId}, ${OVERVIEW_CTX.vds}, 'reference', ${ctxOrder++},
      ...
      if (OVERVIEW_CTX.serviceMac) {
        await tx`
          INSERT INTO skill_context (id, skill_id, context_id, role, sort_order, notes)
          VALUES (${crypto.randomUUID()}, ${skillId}, ${OVERVIEW_CTX.serviceMac}, 'reference', ${ctxOrder++},
      ...
      if (OVERVIEW_CTX.mortgageServicing) {
        await tx`
          INSERT INTO skill_context (id, skill_id, context_id, role, sort_order, notes)
          VALUES (${crypto.randomUUID()}, ${skillId}, ${OVERVIEW_CTX.mortgageServicing}, 'reference', ${ctxOrder++},
      ...
      if (METHODOLOGY_CTX.criticalRules) {
        await tx`
          INSERT INTO skill_context (id, skill_id, context_id, role, sort_order, notes)
          VALUES (${crypto.randomUUID()}, ${skillId}, ${METHODOLOGY_CTX.criticalRules}, 'reference', ${ctxOrder++},
      ...
      if (METHODOLOGY_CTX.patterns) {
        await tx`
          INSERT INTO skill_context (id, skill_id, context_id, role, sort_order, notes)
          VALUES (${crypto.randomUUID()}, ${skillId}, ${METHODOLOGY_CTX.patterns}, 'reference', ${ctxOrder++},
      ...
      if (METHODOLOGY_CTX.mappingDecisions) {
        await tx`
          INSERT INTO skill_context (id, skill_id, context_id, role, sort_order, notes)
          VALUES (${crypto.randomUUID()}, ${skillId}, ${METHODOLOGY_CTX.mappingDecisions}, 'reference', ${ctxOrder++},
      ...
      if (METHODOLOGY_CTX.tableRelationships) {
        await tx`
          INSERT INTO skill_context (id, skill_id, context_id, role, sort_order, notes)
          VALUES (${crypto.randomUUID()}, ${skillId}, ${METHODOLOGY_CTX.tableRelationships}, 'reference', ${ctxOrder++},
      ...
      for (const ctxId of mortgageCtxIds) {
        ...
        await tx`
          INSERT INTO skill_context (id, skill_id, context_id, role, sort_order, notes)
          VALUES (${crypto.randomUUID()}, ${skillId}, ${ctxId}, 'supplementary', ${ctxOrder++},
      ...
      for (const qaId of qaIds) {
        ...
        await tx`
          INSERT INTO skill_context (id, skill_id, context_id, role, sort_order, notes)
          VALUES (${crypto.randomUUID()}, ${skillId}, ${qaId}, 'reference', ${ctxOrder++},
                  ${"Prior Q&A: " + qaName})
        `;
        linkCount++;
      }
